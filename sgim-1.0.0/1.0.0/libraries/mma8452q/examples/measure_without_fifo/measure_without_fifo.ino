/*
 * MMA8452Q Sample sketch for CAIM -- Read 3-axis accelarations without FIFO
 */
...
void loop() {
  ...
  if (status & m8STATUS_ZYXD) {
    ...
    int ax = acc.tomg(acc.toRaw(data[0], data[1]), ACC_RANGE);
    int ay = acc.tomg(acc.toRaw(data[2], data[3]), ACC_RANGE);
    int az = acc.tomg(acc.toRaw(data[4], data[5]), ACC_RANGE);
    char  line[60];
